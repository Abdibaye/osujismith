...
// Helper function to recursively pass visible prop to all children
const passVisibleToChildren = (children: React.ReactNode, visible: boolean): React.ReactNode => {
  return React.Children.map(children, (child) => {
    if (!React.isValidElement(child)) {
      return child;
    }

    // If the child has children, recursively pass visible to them too
    const childElement = child as React.ReactElement<any>;
    const newProps: any = { visible };

    if (childElement.props.children) {
      newProps.children = passVisibleToChildren(childElement.props.children, visible);
    }
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    ...
    >
      {passVisibleToChildren(children, visible ?? false)}
    </motion.div>
  );
};
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible ? "text-zinc-600 hover:text-zinc-800" : "text-white hover:text-gray-200",
        className,
      )}
    >
      {items.map((item, idx) => (
